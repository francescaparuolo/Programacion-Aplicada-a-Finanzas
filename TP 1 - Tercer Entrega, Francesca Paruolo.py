...
class myarray():
    ...
    def __pow__(self, b):
        ...
        if self.f != self.c:
            ...
            while count < b:
                ...
                count += 1
            salida = myarray(matriz_multiplicada, self.f, self.c)
        else:
            print ('El parametro de entrada es de tipo ', str(type(b)))
            salida = None
        return salida
    ...
    def gauss(self):
        '''Función que encuentra la inversa de una matriz.'''
        # 'matriz' va a ser alterada hasta ser igual a la identidad.
        # 'identidad' es la matriz identidad que NO cambia. 
        # 'inversa' es la matriz identidad al comienzo, pero luego va a ser alterada
        #   al igual que la matriz hasta ser la iinversa. 
        matriz = self
        if matriz.f != matriz.c:
            print ('Esta matriz no tiene inversa debido a que no es cuadrada.')
            salida = None
        identidad = matriz.identidad()
        inversa = self.__class__(identidad.elems.copy(), self.f, self.c)
        if identidad.elems == matriz.elems:
            salida = self.__class__(matriz.elems,self.f,self.c)
        for posicion in range(1, self.c+1): # 1, 2, 3 (numero de cada columna)
            fila = posicion+1 # Fila que sirve para agarrar una fila y swapearla con la que tiene un cero. 
            while matriz.get_col(posicion)[posicion-1] == 0 and fila != self.f+1:
                matriz = matriz.swap_rows(posicion, fila)
                inversa = inversa.swap_rows(posicion,fila)
                fila += 1
                if fila == self.f+2: # Esto es para chequear, si no se puede encontrar una fila que no comience con un 0 significa que la matriz NO tiene inversa. 
                    print ('Esta matriz no tiene inversa.')
                    salida = None

            inversa = inversa.scale_row(posicion,(1/matriz.get_elem(posicion, posicion)))
            matriz = matriz.scale_row(posicion,(1/matriz.get_elem(posicion, posicion)))

            inversa = self.matriz_auxiliar_gauss(matriz, posicion) @ inversa
            matriz = self.matriz_auxiliar_gauss(matriz, posicion) @ matriz
            
        salida = inversa
        return salida
            
    def matriz_auxiliar_gauss(self, matriz, columna):
        identidad = self.identidad()
        for posicion, elemento in enumerate(identidad.elems[columna-1::c]):
            if elemento != 1:
                identidad.elems[columna+posicion*identidad.c-1] = (-matriz.elems[columna+posicion*identidad.c-1])
        return identidad

elems = [0, 2, 3, 4, 5, 6, 7, 8, 9]
f = 3
c = 3
x = myarray(elems, f, c)
...
print (f'La matriz inversa de la funcion es: {x.gauss().elems}.')
